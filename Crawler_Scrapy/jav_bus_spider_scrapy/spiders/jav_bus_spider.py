...
from run import START_URLS, API_URL, ALLOWED_DOMAINS
...
class JavBusSpiderSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        url_list = list(map(lambda x: x.split('/')[-1], sample_waterfall))
        ...
        for each_script in script:
            ...
            if "gid" in each_script and "img" in each_script and "uc" in each_script:
                ...
                break
        yield Request(
            self.api_url.format(gid=gid, img=img, uc=uc, floor=random.randint(1, 1000)),
            callback=self.parse_api,
            meta=meta,
            headers={
                'referer': 'https://www.{}/{}'.format(ALLOWED_DOMAINS[0], designation),
    ...
    def parse_api(response):
        ...
        item = JavSpiderByScrapyItem(
            ...
            sample_count=response.meta.get('sample_count'),
            magnet_count=len(magnet_infos)
        )
        yield item
...
class JBStarSpider(JavBusSpiderSpider):
    ...
    start_urls = START_URLS
    rules = (
        Rule(LinkExtractor(allow=start_urls[0] + r'/\d+$'), follow=True),
        Rule(LinkExtractor(allow=r'.+-.+'), callback='parse_item', follow=False)
    )
    cookie = {"existmag": "all",
              " __cfduid": "9b022706b222e4ff027d641edc17b1560240493",
              " PHPSESSID": "op49a7mhrkvfm324lb2qnpf8e0"}
    headers = {
        ...
        'Host': "www.{}".format(ALLOWED_DOMAINS[0]),
